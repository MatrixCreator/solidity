...
void CodeTransform::operator()(FunctionCall const& _call)
{
	...
	EVMAssembly::LabelID returnLabel; // only used for evm 1.0
	if (!m_evm15)
	{
...
void CodeTransform::operator()(FunctionDefinition const& _function)
{
	...
	CodeTransform(m_errors, m_assembly, m_info, m_evm15, m_identifierAccess, 0).run(_function.body);
...
{
	CodeTransform(m_errors, m_assembly, m_info, m_evm15, m_identifierAccess, m_initialStackHeight).run(_block);
}
